...
const make = (overrides = {}) => {
  ...
  const config = { deciderMode: 'demo', defaultDeferMs: 900_000, allowedActions: new Set(['silent','defer','note','webhook']), minActionGapMs: 300_000, dailyActionLimit: 12, webhookUrl: '', ...overrides };
...
test('一次唤醒形成完整可关联流水', async () => {
  ...
  assert.deepEqual(events.map((e) => e.type).reverse(), ['trigger.received','observation.completed','decision.made','governor.allowed','action.completed']);
  assert.equal(new Set(events.map((e) => e.cycle_id)).size, 1);
});
